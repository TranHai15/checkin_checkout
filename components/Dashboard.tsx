...
import { Users, UserCheck, UserMinus, Clock, TrendingUp, AlertTriangle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ stats, recentActivity, lateEmployees }) => {
  return (
              ...
              recentActivity.map((act, idx) => (
                <div key={idx} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-3">
                    <div className={`w-2 h-2 rounded-full ${act.type === 'in' ? 'bg-emerald-500' : 'bg-purple-500'}`}></div>
                    <span className="font-medium text-gray-700">{act.employeeName}</span>
                  </div>
                  <div className="flex items-center space-x-4">
                    ...
                    <span className="text-sm font-mono text-gray-500">{formatTime(act.time)}</span>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="divide-y divide-gray-50 max-h-[300px] overflow-y-auto">
            {lateEmployees.length === 0 ? (
              <div className="p-8 text-center text-gray-400 text-sm">Hôm nay không có ai đi trễ!</div>
            ) : (
              lateEmployees.map((emp, idx) => (
                <div key={idx} className="px-6 py-3 flex items-center justify-between hover:bg-orange-50/20">
